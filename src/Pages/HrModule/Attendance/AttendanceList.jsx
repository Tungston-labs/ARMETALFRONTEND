...
const AttendanceList = () => {
  ...
  const { attendanceList, pagination, listLoading, error } = useSelector(
    (state) => state.attendance,
  );

  const { minList: departments, loading: departmentLoading } = useSelector(
  ...
  if (status) {
    visibleRows = visibleRows.filter((row) => {
      if (status.toLowerCase() === "present")
        return row.attendanceToday === true;
      if (status.toLowerCase() === "absent")
        return row.attendanceToday === false;
